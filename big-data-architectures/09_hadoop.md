# Hadoop
Apache Hadoop è un **sistema per l'implementazione ed esecuzione su cluster di applicazioni data-intensive distribuite, scalabili, disponibili e affidabili**.

Fu inizialmente sviluppato da Yahoo! con lo scopo di fornire storage e processing per le proprie applicazioni data-intensive.
...
## Ecosistema Hadoop
I componenti principali di Hadoop sono *HDFS* (data storage), *Hadoop YARN* (resource management) e *Hadoop MapReduce* (data processing).

Dopo il rilascio della Apache Software Foundation nel 2011, si è sviluppato un vero e proprio **ecosistema Hadoop**, ovvero un *parco di tecnologie Big Data Hadoop-based*.

Ad oggi questo ecosistema comprende tecnologie di:
* resource management
...
## MapReduce
MapReduce è un **modello computazionale divide-et-impera** ispirato alla programmazione Lisp.

La computazione è definita come una **pipeline distribuita di job**, ognuno dei quali è costituito da:

* **Map Task (Mapper):** partizionamento della computazione; è una funzione *map(key1, value1) => list(key2, intermediate_value)* che riceve in input uno shard di entry key-value e produce una lista di entry key-value. Il numero di Mapper deve essere scelto tenendo in considerazione la dimensione dei blocchi HDFS dell'input. I valori intermedi prodotti dai Mapper sono sottoposti dal **Master Controller** ad una fase di **shuffle grouping and sort** nella quale le entry aventi la stessa chiave sono raggruppate ed inviate ordinate ad uno stesso Reducer. Lo spazio delle chiavi intermedie è dunque partizionato tra i Reducer.

* **Combine Task (Combiner):** aggregazione parziale e opzionale dell'output del Mapper; esegue uno *step reduce parziale in-memory* dei dati nello stesso nodo del Mapper al fine di minimizzare la comunicazione tra Mapper e Reducer.

* **Reduce Task (Reducer):** aggregazione totale dei risultati parziali prodotti dal Mapper/Combiner; è una funzione *reduce(key2, list(intermediate_value)) => list(key3, value3)* che riceve in input la lista di valori intermedi per una specifica chiave e produce una lista di entry key-value. Il numero di Reducer è una scelta che deve essere condotta euristicamente.

I dati di input sono partizionati in *M* **Shard**, dove *M* è il numero dei Mapper. L'output di un job è memorizzato nel sottostante DFS: ogni Reducer produce una parte di output.

Tra Mapper (dopo eventuale Combiner) e Reducer vi è un **Partitioner** customizzabile per il mapping dello spazio delle chiavi sullo spazio dei Reducer. Il fatto che sia customizzabile è utile per implementare *politiche ad-hoc di load-balancing e ordinamento*.
...
Il sistema espone una API che rende trasparente il cluster, facendosi carico di:
...
* l'esecuzione del Combiner non è mai assicurata.
...
* *il modello di programmazione è rigido* se usato per applicazioni multi-stage complesse (e.g. graph analysis e machine learning);
* non sfrutta appieno le potenzialità della memoria.

---

### Hadoop Design Patterns
Un **design pattern** è un template che risolve una manipolazione di dati di valenza generale.
L'introduzione di questo concetto si deve a *"Design Patterns: elements of reusable object oriented software" della Gang Of Four*.
...
* inadeguatezza per applicazioni *OnLine Analytical Processing (OLAP)*.
...
Il motivo principale per cui è stato sviluppato è la necessità di rendere navigabili i dati provenienti da applicazioni data-intensive come Hadoop MapReduce a coloro che sono abituati al modello relazionale.
...
Un *workflow* è un insieme di azioni organizzate in una *pipeline DAG* espressa in modo dichiarativo attraverso uno **script PDL (Process Definition Language)** (XML-like) e  *parametrizzabile in funzione di variabili custom*.

I nodi del DAG possono essere:

* **Control Node:** responsabili del controllo di flusso (start, end, failure, fork, join,...);
