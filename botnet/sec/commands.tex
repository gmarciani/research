...
In the command listed below there are non primitive data types. Some of them have been already mentioned in \Cref{sec:bot-configuration}. We report here them too, for reader's convenience.
...
\begin{description}
  ...
  If specified, \texttt{proxy} is the HTTP proxy to attack through. If set to \texttt{null}, it is the default proxy (see \Cref{Sec:bot-configuration}).
...
Every command starts with a field pointing out the command scope, and give the expected parameters.
