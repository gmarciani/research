...
The bot life cycle is modeled as a Finite State Automaton (FSA) with the state space defined as follows and depicted in \Cref{fig:bot-fsa}.
...
\begin{description}
  ...
  Errors in this state are considered fatal, so they would cause bot termination. The pseudocode of the state is shown in \Cref{alg:state-init-pseudocode}.
  ...
  Errors in this state are considered warning: they are handled and never cause bot termination. The pseudocode of the state is shown in \Cref{alg:state-execution-pseudocode}.
  ...
  In this state errors are considered warning: they are handled and never cause bot termination. The pseudocode of the state is shown in \Cref{alg:state-sleep-pseudocode}.

  \item[DEAD] attacks are unscheduled, resources are released and the bot terminates. In this state all errors are ignored, because no one of them could never compromise the state purpose. The pseudocode of the state is shown in \Cref{alg:state-dead-pseudocode}.
