...
The bot has been realized as a Maven-based Java desktop application, packaged into a self-contained Jar with shrunk and obfuscated code (\Cref{sec:concealment}).

Basically, the bot implements the finite state automaton (\Cref{sec:bot}) and the functionalities required for configuration parsing (\Cref{sec:configuration}), commands execution (\Cref{sec:commands}) and logging (\Cref{sec:logging}).
